...
Let's install the Felix Shell bundles (NB these bundles work just find on Equinox and Knopflerfish also):
...
For example suppose our parent application contains a domain model containing Customers and Orders, both of which in the package @org.example.mydomain@. This package needs to be added to the system bundle exports, so go back to the first code sample and insert the following where we had the TODO marker:
