...
Now let's discuss possible solutions. To be clear, all of the following suggestions are intended to supplement rather than replace existing approaches. *If your framework is working in J2SE/J2EE with @Class.forName()@ or TCCLs then you should not continue to use those approaches*. But, please provide additional options for runtime environments such as OSGi where those approaches fail.
