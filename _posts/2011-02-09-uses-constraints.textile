...
Notice how the shaded area avoids the import of foo inside bundle B -- this is only possible if the package bar exported by B has no internal dependency on foo. However if package bar _does_ depend on package foo, then we have a "uses constraint". We illustrate this with a little rubber band, like so:
